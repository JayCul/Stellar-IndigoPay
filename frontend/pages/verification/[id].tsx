...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/verification-requests/${id}/public`);
  if (!res.ok) {
...
export default function VerificationPage({ verification }: Props) {
  ...
  useEffect(() => {
    ...
    if (verification.reviewedAt) {
      const label = verification.status === 'approved' ? 'Approved' : verification.status === 'rejected' ? 'Rejected' : 'Reviewed';
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={`Verification request for project ${verification.projectName}`} />
      </Head>
      <PageMeta title={`Verification #${verification.id}`} description={verification.projectDescription ?? ''} />
      <section className="max-w-4xl mx-auto p-6">
        <h1 className="text-3xl font-bold mb-4">Verification Request – #{verification.id}</h1>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Project</dt>
              <dd className="mt-1 text-lg font-semibold text-gray-900 dark:text-gray-100">{verification.projectName}</dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Status</dt>
              <dd className="mt-1">
                <span className={`px-2 py-1 rounded-full text-sm font-medium ${verification.status === 'approved' ? 'bg-green-100 text-green-800' : verification.status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
            ...
            <div>
              ...
              <dd className="mt-1 text-gray-900 dark:text-gray-100">{verification.walletAddress}</dd>
            </div>
            {verification.reviewedBy && (
              <div>
                <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Reviewed By</dt>
                <dd className="mt-1 text-gray-900 dark:text-gray-100">{verification.reviewedBy}</dd>
              </div>
            )}
          </dl>
        </div>
